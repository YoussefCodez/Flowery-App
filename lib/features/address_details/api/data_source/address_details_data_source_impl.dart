...
class AddressDetailsDataSourceImpl implements AddressDetailsDataSourceContract {
  ...
  Future<Result<AddressDetailsResponce>> updateAddressDetails(
    ...
    try {
      ...
      return Error<AddressDetailsResponce>(
        exception: Exception(e.response?.data?[AddressDetailsValues.message]),
      );
    }
  }
  ...
  Future<Result<AddressDetailsEntity>> getSavedAddresses() async {
    try {
      final response = await apiClient.getSavedAddresses();
      return Success<AddressDetailsEntity>(data: response.toDomain());
    } on DioException catch (e) {
      return Error<AddressDetailsEntity>(
        exception: Exception(e.response?.data?[AddressDetailsValues.message]),
      );
    }
  }
  ...
  @override
  Future<Result<AddressDetailsResponce>> deleteAddress(String addressId) async {
    try {
      final response = await apiClient.deleteAddress(addressId);
      return Success<AddressDetailsResponce>(data: response);
    } on DioException catch (e) {
      return Error<AddressDetailsResponce>(
        exception: Exception(e.response?.data?[AddressDetailsValues.message]),
